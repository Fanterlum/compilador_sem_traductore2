...
//Funcion que separa las palabras de la cadena
...
void Lexico::entrada(string fuente){
     ind= 0;//inicializamos el indice en 0           
     this->fuente= fuente;//asignamos la cadena a la variable fuente
}

int Lexico::sigSimbolo(){
     estado=0; //inicializamos el estado en 0
     continua= true; 
     simbolo= "";
     bool entero = true;
     bool reales = true;
     ...
     while (continua){
          ...
          switch (estado){
               case 0:
                    ...
                    //si no es ninguno de los anteriores enctoces es un identificador o un numero
                    for (int i=0; i < longitud ; i++){ 
                         bool x = isdigit(c[i]);
                         if(x == 0 && c[i] != '-'){entero = false;}//si uno de los caracteres no es digito y no es un guion no es un entero
                         if(x == 0 && c[i] != '.' && c[i] != '-')reales = false;//si uno de los caracteres no es digito, ni es punto, ni es guion no es un real
                         if(x == 0 && c[i] == '\"')cadena = true;//si uno de los caracteres no es digito y es comilla doble entonces es una cadena
                    }
                    if (entero == true){ 
                         aceptacion(4);
                         break;
                    ...
                    else{//si no es ninguno de los anteriores entonces es un identificador
                         aceptacion(1);
                         break;
                    }
               break;                       
...
string Lexico::sigCaracter(){//regresa el siguiente caracter de la cadena
     string palabra;
     if (terminado()){ 
          return "$";
     }
     //si hay simbolo de $ entonces se regresa el simbolo
     if(fuente[ind] == '$'){
          return "$";
     }
     //si no hay simbolo de $ entonces se regresa la palabra
     // se regresa la palabra y se continua con la siguiente
     else{
          while(fuente[ind] != ' '){
               palabra += fuente[ind];
               ind++;
...
void Lexico::sigEstado(int estado){//asigna el estado siguiente y el simbolo que se esta analizando
     this->estado = estado;
     simbolo += c;
}


void Lexico::aceptacion(int estado){//asigna el estado de aceptacion y termina el automata
     sigEstado(estado);
     continua= false;
}

bool Lexico::terminado(){//revisa si ya se llego al final de la cadena
